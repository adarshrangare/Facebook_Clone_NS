    ...
    <div className="mx-auto w-full h-[92%] rounded-lg pb-6 overflow-y-auto space-y-4">
      {pages?.map((page) => (
        ...
        >
          ...
          <button className="w-full  py-2 shadow-md  bg-blue-100/65 hover:bg-blue-100  dark:bg-blue-100/10 dark:hover:bg-blue-100/20 text-blue-500 font-semibold mt-2 rounded-md ">
            <Link href={`/page/${page._id}`}>
              {" "}
              <RiSearchEyeLine className="mx-1 inline" size={18} /> View Page
            </Link>
          </button>
        </div>
      ))}
